...
def request_html_by_requests(url, proxies):
    ...
    try:
        res = requests.get(url, headers=headers, timeout=CONFIG.TEST_URL['timeout'], proxies=proxies)
...
async def valid_proxies(ip, port):
    ...
    html = await request_html_by_aiohttp(url=CONFIG.TEST_URL['http'], proxy=proxy, timeout=CONFIG.TEST_URL['timeout'])
    if html:
        try:
