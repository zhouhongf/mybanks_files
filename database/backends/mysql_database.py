...
import os
from .mongo_database import update_myworld_pdfs
...
class MySQLDatabase(object):
    ...
    MYSQLDB = Settings.mysqldb_config
...
def dump_data_out():
    target_path = os.path.join(CONFIG.DATA_DIR, 'dumps')
    os.makedirs(target_path, exist_ok=True)

    sql = 'select * from crawler_manual'
    db = MySQLDatabase()
    dataList = db.query(sql)

    # for one in dataList:
    #    print('ukey是：%s, createTime是：%s' % (one['ukey'], one['create_time']))

    update_myworld_pdfs(dataList)


def dump_pcds_out():
    mongo = MongoDatabase().client()
    db_target = mongo['ubank']
    collection = db_target['pcds']
